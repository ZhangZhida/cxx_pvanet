...
void PVADetector::drawBox(cv::Mat& img, std::vector<Detection> &dets,int Trans_x,int Trans_y)
{
    for (int i = 0; i<dets.size(); ++i)
    {
        Rect old_rect = dets[i].getRect();
        Rect new_rect(old_rect.x+Trans_x,old_rect.y+Trans_y,old_rect.width,old_rect.height);
        rectangle(img, new_rect, cv::Scalar(0, 0, 255), 2);
        std::ostringstream os;
        os.precision(2); //保留2位有效数字
        os << dets[i].getScore();
        std::string showTxt = dets[i].getClass(); //+ "," + os.str();;
        putText(img, showTxt, // Mat, text
                cv::Point(dets[i].getRect().x+Trans_x, dets[i].getRect().y+Trans_y - 2), //oriPt
...
void PVADetector::drawBox(cv::Mat& img, std::vector<Detection> &dets,string type)
{
    for (int i = 0; i<dets.size(); ++i)
    {
        std::ostringstream os;
        ...
        if(type==showTxt){
            ...
            putText(img, showTxt, // Mat, text
                    ...
                    cv::FONT_HERSHEY_SIMPLEX, 1, //Font
                    cv::Scalar(0,255, 255), 1); // color, linear width
            rectangle(img, dets[i].getRect(), cv::Scalar(0, 255,0), 2);
        }
    }
}
